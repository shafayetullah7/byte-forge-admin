...
export default function CategoryManagementHub() {
    ...
    const [newSubNameBn, setNewSubNameBn] = createSignal("");
    const [newSubDescEn, setNewSubDescEn] = createSignal("");
    const [newSubDescBn, setNewSubDescBn] = createSignal("");
    ...
    const handleSaveChanges = async () => {
        ...
        // When saving general changes, we also update the English translation to match
        ...
        const translations = [
            {
                ...
                description: newSubDescEn().trim() || null
            }
        ];

        if (newSubNameBn().trim()) {
            translations.push({
                locale: "bn",
                name: newSubNameBn().trim(),
                description: newSubDescBn().trim() || null
            });
        }
                                    ...
                                    <div class="lg:col-span-2 space-y-8">
                                                ...
                                                <div class="mb-6 p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                                                    ...
                                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                        ...
                                                        <Input
                                                            ...
                                                            placeholder="উদা: গেমিং ল্যাপটপ"
                                                            maxLength={255}
                                                            value={newSubNameBn()}
                                                            onInput={(e) => setNewSubNameBn(e.currentTarget.value)}
                                                        />
                                                        <Input
                                                            label="Slug *"
                                                            class="md:col-span-2"
                                                        ...
                                                        <div class="md:col-span-2 space-y-1">
                                                            <label class="text-[11px] font-bold text-slate-500 uppercase tracking-wider ml-1">Subcategory Description (EN)</label>
                                                            <textarea
                                                                placeholder="e.g. High-performance gaming laptops with dedicated graphics..."
                                                                maxLength={1000}
                                                                class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-white text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary-green-500/20 focus:border-primary-green-500 transition-all min-h-[80px]"
                                                                value={newSubDescEn()}
                                                                onInput={(e) => setNewSubDescEn(e.currentTarget.value)}
                                                            />
                                                        </div>
                                                        <div class="md:col-span-2 space-y-1">
                                                            <label class="text-[11px] font-bold text-slate-500 uppercase tracking-wider ml-1">Subcategory Description (BN)</label>
                                                            <textarea
                                                                placeholder="উদা: ডেডিকেটেড গ্রাফিক্স সহ উচ্চ-ক্ষমতাসম্পন্ন গেমিং ল্যাপটপ..."
                                                                maxLength={1000}
                                                                class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-white text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary-green-500/20 focus:border-primary-green-500 transition-all min-h-[80px]"
                                                                value={newSubDescBn()}
                                                                onInput={(e) => setNewSubDescBn(e.currentTarget.value)}
                                                            />
                                                        </div>
                                                    </div>
                                        ...
                                        <Card class="p-6">
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                                                ...
                                                <CategorySelector
                                                    label="Change Parent Category"
                                                    categories={categoriesTree() || []}
                                                    value={editParentId()}
                                                    onChange={setEditParentId}
                                                    disabledId={params.category_id}
                                                />
                                            </div>
                                        </Card>
