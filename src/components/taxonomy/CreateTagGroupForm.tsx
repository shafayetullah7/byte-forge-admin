...
export function CreateTagGroupForm(props: CreateTagGroupFormProps = {}) {
    ...
    // Tag input states
    const [tagFormState, setTagFormState] = createStore({
        tagInput: "",
        tagInputBn: "",
        tagSlugInput: "",
        isTagSlugManual: false,
        isSlugManual: false
    });
    ...
    const handleTagInput = (e: Event) => {
        ...
        console.log("Valueeeeeeeeeeeeeeeeeee", val)
        setTagFormState("tagInput", val);
        if (!tagFormState.isTagSlugManual && val) {
            setTagFormState("tagSlugInput", slugify(val));
        } else if (!tagFormState.isTagSlugManual && !val) {
            setTagFormState("tagSlugInput", "");
        }
    };

    // Handle tag slug input
    const handleTagSlugInput = (e: Event) => {
        setTagFormState("tagSlugInput", (e.currentTarget as HTMLInputElement).value);
        setTagFormState("isTagSlugManual", true);
    };

    // Handle Bengali tag input
    const handleTagInputBn = (e: Event) => {
        setTagFormState("tagInputBn", (e.currentTarget as HTMLInputElement).value);
    };

    // Add a new tag to the form
    const handleAddTag = () => {
        const nameEn = tagFormState.tagInput.trim();
        const nameBn = tagFormState.tagInputBn.trim();
        const slugVal = tagFormState.tagSlugInput.trim();
        // @ts-expect-error - modular-forms doesn't support getting entire array fields, only nested paths
        const currentTags = (getValue(tagGroupForm, "tags") || []) as { slug: string; isActive?: boolean; translations?: { locale: string; name: string }[] }[];

        console.log(currentTags)

        if (nameEn && nameBn && slugVal && !currentTags.some(t => t.slug === slugVal)) {
            // @ts-expect-error - modular-forms doesn't support setting entire array fields, only nested paths
            setValue(tagGroupForm, "tags", [...currentTags, {
                slug: slugVal,
                isActive: true,
                translations: [
            ...
            setTagFormState({
                ...
                tagSlugInput: "",
                isTagSlugManual: false,
                isSlugManual: tagFormState.isSlugManual
            });
        }
    };
    ...
    const handleRemoveTag = (tagSlug: string) => {
        // @ts-expect-error - modular-forms doesn't support getting entire array fields, only nested paths
        const currentTags = (getValue(tagGroupForm, "tags") || []) as { slug: string }[];
        // @ts-expect-error - modular-forms doesn't support setting entire array fields, only nested paths
        setValue(tagGroupForm, "tags", currentTags.filter(t => t.slug !== tagSlug));
    };
                        ...
                        <Field name="translations.0.name">
                            {(field, fieldProps) => {
                                const handleNameChange = (e: Event) => {
                                    ...
                                    if (!tagFormState.isSlugManual && newName) {
                                        setValue(tagGroupForm, "slug", slugify(newName));
                                    } else if (!tagFormState.isSlugManual && !newName) {
                        ...
                        <Field name="slug">
                            {(field, fieldProps) => {
                                const handleSlugChange = (e: Event) => {
                                    ...
                                    setTagFormState("isSlugManual", true);
                                };
                ...
                <Card class="p-6">
                    <div class="flex items-start justify-between mb-6">
                        ...
                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-primary-green-50 text-primary-green-700 border border-primary-green-100">
                            {/* @ts-expect-error - modular-forms doesn't support getting entire array fields, only nested paths */}
                            {((getValue(tagGroupForm, "tags") as unknown as { slug: string }[])?.length || 0)} Tags
                        </span>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 items-end mb-6">
                        <div class="w-full sm:flex-1">
                            <Input
                                label="Tag Slug *"
                                placeholder="e.g. high"
                                value={tagFormState.tagSlugInput}
                        ...
                        <Button
                            ...
                            disabled={!tagFormState.tagInput.trim() || !tagFormState.tagInputBn.trim() || !tagFormState.tagSlugInput.trim() || isPending()}
                            ...
                            <div class={`p-6 rounded-xl border border-dashed bg-slate-50/50 min-h-[200px] flex flex-wrap gap-2.5 content-start ${fieldArray.error ? "border-red-500 bg-red-50/10" : "border-slate-200"}`}>
                                ...
                                >
                                    <For each={fieldArray.items}>
                                        {(item, index) => {
                                            const tagSlug = () => getValue(tagGroupForm, `tags.${index()}.slug`) as string;
                                            const tagNameEn = () => getValue(tagGroupForm, `tags.${index()}.translations.0.name`) as string;
                                            const tagNameBn = () => getValue(tagGroupForm, `tags.${index()}.translations.1.name`) as string;
                                            ...
                                            return (
                                                <div class="inline-flex flex-col gap-1 pl-3 pr-2 py-2 rounded-lg bg-white border border-slate-200 shadow-sm transition-all hover:border-primary-green-300">
                                                    ...
                                                    <span class="text-[10px] text-slate-400 font-mono bg-slate-50 px-1 py-0.5 rounded w-max">{tagSlug()}</span>
                                                </div>
                                            );
                                        }}
                                    </For>
