...
import type { TagGroup, CreateTagGroupDto } from "~/lib/api/endpoints/tag-groups/tag-groups.types";
...
export function CreateTagGroupForm(props: CreateTagGroupFormProps = {}) {
    ...
    const handleAddTag = () => {
        ...
        const currentTags = (getValue(tagGroupForm, "tags" as any) || []) as { slug: string; isActive?: boolean; translations?: { locale: string; name: string }[] }[];
    ...
    const handleRemoveTag = (tagSlug: string) => {
        const currentTags = (getValue(tagGroupForm, "tags" as any) || []) as { slug: string }[];
        setValue(tagGroupForm, "tags" as any, currentTags.filter(t => t.slug !== tagSlug));
    };
